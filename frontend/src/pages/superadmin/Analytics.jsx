...
export default function Analytics() {
  ...
  const subjectCounts = {};
  tutors.forEach(t => (t.subjects || []).forEach(s => { subjectCounts[s] = (subjectCounts[s] || 0) + 1; }));
  ...
  return (
    <div className="sa-page">
      <div>
        <p className="sa-kicker">Insights</p>
        <h1 className="sa-section-title" style={{ fontSize: '1.5rem' }}>Analytics</h1>
      </div>

      {/* Summary Stats */}
      <div className="sa-stats-grid" style={{ marginTop: '1.5rem' }}>
        <div className="sa-stat"><p className="sa-stat__label">Total Tutors</p><p className="sa-stat__value" style={{ color: '#83B822' }}>{tutors.length}</p></div>
        <div className="sa-stat"><p className="sa-stat__label">Verified</p><p className="sa-stat__value" style={{ color: '#5722B8' }}>{tutors.filter(t => t.verified).length}</p></div>
        <div className="sa-stat"><p className="sa-stat__label">Pending</p><p className="sa-stat__value" style={{ color: '#f59e0b' }}>{tutors.filter(t => !t.verified).length}</p></div>
        <div className="sa-stat"><p className="sa-stat__label">Total Students</p><p className="sa-stat__value">{students.length}</p></div>
      </div>
      ...
      <div className="sa-charts-grid">
        <section className="sa-chart-card">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={subjectData} margin={{ top: 5, right: 10, left: 0, bottom: 40 }}>
        ...
        <section className="sa-chart-card">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={verificationData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
